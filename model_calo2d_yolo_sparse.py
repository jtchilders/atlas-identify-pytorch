...
class Net2D(nn.Module):
   def __init__(self, input_spatial_shape=[256, 5760], input_channels=2):
      ...
      self.pooling_factor = 2 ** 5
      self.grid = [int(i / self.pooling_factor) for i in input_spatial_shape]
      logger.info('grid = %s',self.grid)

      self.block_pars_preconnect = [
         {'type':'full','opts':{'nIn':   2,'nOut':  32,'filter_size':(3,3),'pool_size':(2,2)}},  # 1
         {'type':'full','opts':{'nIn':  32,'nOut':  64,'filter_size':(3,3),'pool_size':(2,2)}},  # 2
         {'type':'part','opts':{'nIn':  64,'nOut': 128,'filter_size':(3,3)}},  # 3
         {'type':'part','opts':{'nIn': 128,'nOut':  64,'filter_size':(1,1)}},  # 4
         {'type':'full','opts':{'nIn':  64,'nOut': 128,'filter_size':(3,3),'pool_size':(2,2)}},  # 5
         ...
         {'type':'part','opts':{'nIn': 256,'nOut': 512,'filter_size':(3,3)}},  # 9
         {'type':'part','opts':{'nIn': 512,'nOut': 256,'filter_size':(1,1)}},  # 10
         {'type':'part','opts':{'nIn': 256,'nOut': 512,'filter_size':(3,3)}},  # 11
   ...
   def forward(self,x):
      ...
      # logger.info('output from input_layer: %s',x)
